...
class FileSaver {
  ...
  Future<String> saveAs(String name, Uint8List bytes, String ext, MimeType mimeType) async {
    ...
    // Android ignores the extension property, so we need to append the extension.
    if (Platform.isAndroid && mimeType == MimeType.OTHER) {
      filename += "." + ext;
    }
    Map<dynamic, dynamic> data = {'name': filename, 'ext': ext, 'bytes': bytes, 'type': _mimeType};
